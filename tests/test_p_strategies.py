...
def test_menu_has_exactly_p1_through_p10():
    # No gap and no extra/missing slot (P1–P10 after structural T5→P10).
    assert ALL_P_STRATEGY_IDS == _EXPECTED_IDS
    assert len(P_STRATEGIES) == 10
...
def test_render_menu_lists_all_ten_with_safety_flags():
    menu = render_menu()
    for i in range(1, 11):
