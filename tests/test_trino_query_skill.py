...
from genie.skills.trino_query import TrinoQuerySkill, TrinoExplainSkill, QueryResult, _clean_row
...
class TestSkillRunMocked:
    ...
    def test_query_run_success(self, mock_profile):
        mock_conn = MagicMock()
        mock_cur = MagicMock()
        mock_cur.description = [("name",), ("val",)]
        mock_cur.fetchall.return_value = [("Alice", 100), ("Bob", 200)]
    ...
    def test_explain_run_success(self, mock_profile):
        mock_conn = MagicMock()
        mock_cur = MagicMock()
        mock_cur.fetchall.return_value = [("Fragment 0 [SOURCE]",), ("  Output layout: [a]",)]
        mock_conn.cursor.return_value = mock_cur
        mock_profile.return_value = MagicMock()
        mock_profile.return_value.connect.return_value = mock_conn
