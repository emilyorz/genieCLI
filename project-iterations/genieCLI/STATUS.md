...
- **Ledger:** [CURRENT.md](CURRENT.md) holds **v31** (last _formal_ strict-V3 iteration, DONE). v32 + v33 + v34 + v35 shipped as out-of-process / TLV4-local iterations — see archives.
- **Status:** v31 (formal) + v32 + v33 + v34 + v35 complete and on `main`. **v36 Task-Ledger-V4 Codex bugfix-profile run reached DONE in the working tree; no commit/push has been made.**
- **v36 (TLV4 Codex bugfix profile, DONE in working tree):** repo-local cleanup of v34 memory-limit residuals. Fixed stale `make_query_max_run_time_sql()` docstring, made T-F-24 executable for MCP plan-cost memory-limit threading, and clarified bad env + failed `SHOW SESSION` breadcrumb to state the 1.0 GiB fallback is active. No live office `mcp-trino` / `localhost:8811` / `SHOW SESSION` numeric validation was performed; Sam marked item 1 unavailable for this run, so that remains residual. External `validate_ledger.py` v3 schema support remains out-of-repo. Full suite **935 pass, 1 warning**. Archive: [archive/v36.md](archive/v36.md). V4 evidence: `.tlv4-v36-cleanup/`.
...
- **Last completed iteration:** v36 repo-local memory-limit cleanup for `/trino-research`.
- **Resume action:** If Sam requests landing, commit/push v36. Otherwise continue from the remaining backlog below. `CURRENT.md` still reads v31 (v32/v33/v34/v35/v36 were out-of-process / TLV4-local records).
...
| -------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| trino-research | [features/trino-research.md](features/trino-research.md) | v36 (memory-limit cleanup: timeout docstring, executable MCP plan-cost threading coverage, bad-env fallback breadcrumb; TLV4 Codex DONE) |
| mcp-banner     | [features/mcp-banner.md](features/mcp-banner.md)         | v26 (fast-fail 200ms probe; 14× cold-start improvement)                                                                                      |

## Archive

- [archive/v36.md](archive/v36.md) — repo-local memory-limit cleanup for `/trino-research` (Task-Ledger-V4 Codex bugfix profile DONE): timeout docstring corrected for 2x/2000ms behavior, T-F-24 now executes MCP plan-cost memory-limit threading assertion, bad-env + failed `SHOW SESSION` breadcrumb says 1.0 GiB fallback is active; full suite **935 pass**, 1 warning; no live cluster probe; uncommitted working tree until Sam requests landing
