...
- **Status:** v31 (formal) + v32 + v33 + v34 + v35 + v36 + **v37** complete and on `main`. Local `main` == `origin/main` (synced). Authoritative full suite on `main` HEAD `96fb18f`: **1047 pass, 11 skipped**.
...
- **Last completed iteration:** v37 `trino-optimize` profile (decompose/optimize/recompose) for `/trino-research`.
- **Resume action:** All shipped work is on `main` and pushed (`origin/main` synced). Next planning pass: write the missing `archive/v37.md` retro, document tGenie P1–P8 (P5 gap), then pick from the backlog below. `CURRENT.md` still reads v31 (v32–v37 were out-of-process / TLV4-local records).
...
| -------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| trino-research | [features/trino-research.md](features/trino-research.md) | v37 (`trino-optimize` profile: 3-stage detection + decompose/optimize/recompose pipeline + row-equiv P0 gate + R9 join-first-filter-late rule + no-data optimized-SQL surface) |
