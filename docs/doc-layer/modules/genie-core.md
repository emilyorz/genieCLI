...
---
## Purpose
Shared foundation: config loading/merging, `Provider`/`OutputSink` protocols, singleton `SkillRegistry`/`BaseSkill` plugin contract, token-budget management, SQL lint rules for Oracle-to-Trino migration, tool-call parsing, and model profile metadata.
## Exports
...
## Invariants
- `lint_analyzer` uses a package-relative import `from .lint_rules import ALL_RULES, Finding`; no `sys.path` manipulation required (`lint_analyzer.py:7`).
- `load()` applies layers: JSON legacy → TOML → env vars → CLI overrides; `None` values skipped; malformed files silently ignored (`config.py:57-61`, `config.py:76-81`, `config.py:95-105`).
## Change log
- 572f7ff30399bed1a1a3c230918ba037ae874272: fix fabricated relative-import invariant and wrong line citation for lint_analyzer; Exports regenerated from emitter; budget violation resolved
